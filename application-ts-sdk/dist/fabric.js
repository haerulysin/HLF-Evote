...
const user_json_1 = __importDefault(require("./wallet/user.json"));
...
const createWallet = async (user) => {
    ...
    const userIdentity = user_json_1.default;
    await wallet.put("user", userIdentity);
    return wallet;
};
...
;
async function submitTransaction(contract, transaction, ...transactionArgs) {
    const tx = transaction.getTransactionId();
    try {
